<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter"  :key="index" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index; selectAddId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="delAddressConfrim(item.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a v-if="!item.isDefault" @click="setDefault(item.addressId)" href="javascript:;" class="addr-set-default-btn"><i>Set default</i></a>
                    </div>
            ...
            <div class="next-btn-wrap">
             <!-- <router-link class="btn btn--m btn--red" to="/orderConfirm">Next</router-link> -->
              <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selectAddId}}">Next</router-link>
              
              <!-- <a href="/#/orderConfirm">sss</a> -->
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal :mdShow="isMdShow" v-on:close='closeModal'>
        <p slot='message' style="font-size: 16px;">你确定要删除该地址吗！</p>
        <div slot="btnGroup">
          <a class="btn btn--m" style="float: left;" @click="delAddress">确定</a>
          <a class="btn btn--m" style="float: right;" @click="isMdShow=false">取消</a>
        </div>
      </Modal>
      <!-- 添加地址模态框 -->
      <Modal :mdShow="isAdd" v-on:close='closeModal' class="addAddress">
        <p slot='message'>
          <label>添加新的收货地址：</label>
        ...
        <div slot="btnGroup">
          <a class="btn btn--m" style="float: left;" @click="addAddress">确定</a>
          <a class="btn btn--m" style="float: right;" @click="isAdd=false">取消</a>
        </div>
      </Modal>
      <!-- 底部 -->
      <NavFooter></NavFooter>
...
<script>
  ...
  export default{
      data(){
          return{
            //添加地址的数据start
            isAdd:false,  // 添加地址模态框默认隐藏
            name:'',
            address:'',
            tel:'',
            postCode:'',
            //end
            selectAddId:'un',  // 选中地址id
            addressId:'',  // 定义要删除地址的id，缓存
            isMdShow:false,   // 默认隐藏删除对话框
            checkIndex:0,   // 默认地址选中的索引
            addressList:[],
            limit:3  ,// 默认显示三条数据
            //open:false
          }
      ...
      methods:{
        ...
        delAddressConfrim(addressId){
          ...
          this.addressId=addressId   // 要删除的id，缓存了
        },
        closeModal(){   // 关闭对话框
          this.isMdShow=false
          this.isAdd=false
        ...
        setDefault(addressId){
           //console.log(addressId)
          // get和post什么场合用，按技术实现来说什么都可以，安全与不安全，（一般情况获取用get，设置用post）
          axios.post('/users/setAddress',{addressId:addressId}).then(res=>{
            if(res.data.status=='0'){
               // console.log(res)
               this.init()   // 当设置成功后，刷新页面数据，那时的数据已经是修改成功后的数据
            }
          })
        },
        init(){
          axios.get('/users/addressList').then(res=>{
            ...
            this.addressList.forEach((item,index)=>{
              if(item.isDefault){
                ...
                this.addressList.unshift(tempArr)
                
                this.selectAddId=item.addressId   // 传递到orderConfirm地址里，默认地址
              }
            })
            //console.log(this.addressList)
          })
      ...
      computed:{
        addressListFilter(){
          /* return this.addressList.filter((item,index)=>{
