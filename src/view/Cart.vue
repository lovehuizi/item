<template>
  <div>
    ...
    <div class="container">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                  <!-- 按钮上添加一个class为btn--dis，则这个按钮禁用，目的是当购物车选中商品为0的时候不能用-->
     ...
     <!-- 删除购物车商品对话框 -->
     <Modal :mdShow="modalConfim" v-on:close="closeModal">
       <p slot='message' style="font-size: 16px;">你确定要删除该商品吗！</p>
       <div slot="btnGroup">
         <a class="btn btn--m" @click="delCart" style="float: left;" >确定</a>
         <a class="btn btn--m" @click="modalConfim=false" style="float: right;">取消</a>
       </div>
     </Modal>
    <!-- 底部 -->
    <NavFooter></NavFooter>
...
<script>
  // import {currency} from '../util/currency.js'  // 注意 {函数名}，货币格式化--局部
  ...
  import Modal from '../components/Modal.vue'   // 引入模态框
    export default{
        data(){
            return{
              // checkAllFlag:false,  // 默认全选按钮为不选-修改成计算属性
              cartList:[],
              modalConfim:false   ,// 默认关闭模态框
              productId:'',   // 商品id，全局缓存
              delItem:''//删除商品参数
            }
        },
       /* filters:{   // 可以有多个过滤器
          c:currency   // 过滤器为c，currency过滤器指定的函数
        }, */
        methods:{
          // 跳转选择地址
          checkOut(){
          ...
          // 全选按钮切换状态
          toggleCheckAll(){
            //this.checkAllFlag=!this.checkAllFlag  //报错，因为checkAllFlag其实是个方法，你给方法赋值了，白一点：你给函数赋值（函数可以传递值）
          ...
          // 购物车商品数量+ -
           editCart(flag,item){
             if(flag=='add'){
               ...
               if(item.productNum<=1){
                 return   // 防止继续往下运行，目的防止非法的数据进入数据库
               }
               item.productNum--
               this.$store.commit('updateCartCount',-1)
             }else{
           ...
           delCartConfirm(item){
             this.delItem=item
             this.modalConfim=true  // 显示模态框
             this.productId=item.productId   // 1、显示模态框就给商品id赋值了，将来要删除的数据
           },
           // 当你确定要删除该商品，点击确定按钮
           delCart(){
              //console.log('ok')
           ...
           closeModal(){
             this.modalConfim=false  // 关闭模态框
           },
           init(){   // 初始化函数，调用cartList
